...
export async function createStripeCheckoutSession({
  ...
  // this is stripeconnectid of the owner of the event 
  ...
  const session = await stripe.checkout.sessions.create(
    {
      ...
      payment_intent_data: {
        application_fee_amount: Math.round(event.price * 100 * 0.01),  // we as owner of the platform are taking 1% fee of payment
      },
    ...
    {
      stripeAccount: stripeConnectId,   // stripe connect id of event owner(Seller)
    }
  );
